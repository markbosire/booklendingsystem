...
class BooksBorrowTest < ActionView::TestCase
  ...
  test "renders return book page correctly" do
    render template: "books/return", assigns: @view_assigns

    assert_select "h1", text: "Return Book" do
    ...
    assert_select "h3", text: "Confirm Return" do
      puts "✅ Confirmation section header displayed correctly"
    end

    assert_select "p", text: /You are about to return "#{@book.title}" by #{@book.author}/ do
      puts "✅ Return confirmation message displayed correctly"
    end
    ...
    # Action buttons
    assert_select "form[action=?][method=post]", complete_return_book_path(@book) do
      assert_select "button", text: "Confirm Return" do
        puts "✅ Confirm return button displayed correctly"
      end
    end
